...
TEST_CASE("list process ids - Linux", "[os::linux::process]") {
    REQUIRE_THAT(zero::os::linux::process::all(), Catch::Matchers::Contains(getpid()));
}
...
TEST_CASE("process - Linux", "[os::linux::process]") {
    ...
    auto process = zero::error::guard(zero::os::linux::process::open(static_cast<pid_t>(child.pid())));
