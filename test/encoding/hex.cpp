...
TEST_CASE("hex encoding", "[encoding]") {
    REQUIRE_THAT(zero::encoding::hex::encode(nonstd::span<const std::byte>{}), Catch::Matchers::IsEmpty());
    REQUIRE(zero::encoding::hex::encode(as_bytes(nonstd::span{DATA})) == "68656c6c6f");
}
