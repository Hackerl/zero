...
tl::expected<std::filesystem::path, std::error_code> zero::filesystem::applicationPath() {
#ifdef _WIN32
...
tl::expected<std::filesystem::path, std::error_code> zero::filesystem::applicationDirectory() {
...
zero::filesystem::write(const std::filesystem::path &path, const std::string_view content) {
    return write(path, as_bytes(nonstd::span{content}));
}
...
tl::expected<void, std::error_code> zero::filesystem::createDirectory(const std::filesystem::path &path) {
    std::error_code ec;
...
#ifdef _WIN32
        return tl::unexpected{std::error_code{ERROR_FILE_EXISTS, std::system_category()}};
#else
        return tl::unexpected{std::error_code{EEXIST, std::system_category()}};
#endif
    }
...
tl::expected<void, std::error_code> zero::filesystem::createDirectories(const std::filesystem::path &path) {
    std::error_code ec;
...
tl::expected<void, std::error_code> zero::filesystem::remove(const std::filesystem::path &path) {
    std::error_code ec;
...
#ifdef _WIN32
        return tl::unexpected{std::error_code{ERROR_FILE_NOT_FOUND, std::system_category()}};
#else
        return tl::unexpected{std::error_code{ENOENT, std::system_category()}};
#endif
    }
