...
constexpr auto DECODE_MAP = std::array{
...
std::string zero::encoding::base64::encode(const std::byte *buffer, size_t length) {
...
std::optional<std::vector<std::byte>> zero::encoding::base64::decode(std::string_view encoded) {
    if (encoded.length() % 4)
        return std::nullopt;
    ...
    for (size_t i = 0; i < size; i += 4) {
        ...
        b4[0] = (encoded[i] <= 'z') ? std::byte(DECODE_MAP[encoded[i]]) : std::byte{0xff};
        b4[1] = (encoded[i + 1] <= 'z') ? std::byte(DECODE_MAP[encoded[i + 1]]) : std::byte{0xff};
        b4[2] = (encoded[i + 2] <= 'z') ? std::byte(DECODE_MAP[encoded[i + 2]]) : std::byte{0xff};
        b4[3] = (encoded[i + 3] <= 'z') ? std::byte(DECODE_MAP[encoded[i + 3]]) : std::byte{0xff};
