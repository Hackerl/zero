...
namespace {
    ...
    tl::expected<std::array<HANDLE, 2>, std::error_code>
...
zero::os::process::PseudoConsole::spawn(const Command &command) {
    ...
    ranges::copy(command.mArguments, ranges::back_inserter(arguments));
...
zero::os::process::PseudoConsole::spawn(const Command &command) {
    ...
    ranges::transform(
        envs,
        ranges::back_inserter(environment),
        [](const auto &it) {
    ...
    if (*pid == 0) {
        const auto guard = [fd = fds[1]](auto &&f) {
            static_assert(std::is_integral_v<std::invoke_result_t<decltype(f)>>);
...
zero::os::process::Command::spawn(const std::array<StdioType, 3> &defaultTypes) const {
    ...
    const auto expected = [](auto &&f) -> tl::expected<void, std::error_code> {
        static_assert(std::is_same_v<std::invoke_result_t<decltype(f)>, int>);
        const auto result = f();

        if (result != 0)
            return tl::unexpected{std::error_code{result, std::generic_category()}};
...
tl::expected<zero::os::process::ExitStatus, std::error_code> zero::os::process::Command::status() const {
